...
class HingeNode():
    def __init__(self, 
                 time_slice:int, 
                 top_temp:float, middle_temp:float, bottom_temp:float,
                 thermocline1:float, thermocline2: float, params: DParams):
        # Position in graph
...
class FloHinge():
    ...
    def evaluate_branches(self):
        ...
        print(f"\nThe best path forward is {min(self.feasible_branches, key=lambda k: self.feasible_branches[k]['total_pathcost'])}")

    def follow_branch(self, branch1_charge, branch2_charge, branch3_charge, combination_name):
    ...
    def knit_branches(self):
        for branch in self.feasible_branches:
            ...
            if not nodes_with_same_top:
                if n.middle_temp is not None:
                    kWh_top = (n.thermocline1-0.5)*m_layer_kg * 4.187/3600 * to_kelvin(top_temp)
                    kWh_midlle = (n.thermocline2-n.thermocline1)*m_layer_kg * 4.187/3600 * to_kelvin(n.middle_temp)
                    kWh_bottom = (self.g.params.num_layers-n.thermocline2+0.5)*m_layer_kg * 4.187/3600 * to_kelvin(n.bottom_temp)
                else:        
                    kWh_top = (n.thermocline1-0.5)*m_layer_kg * 4.187/3600 * to_kelvin(top_temp)
                    kWh_midlle = 0
                    kWh_bottom = (self.g.params.num_layers-n.thermocline1+0.5)*m_layer_kg * 4.187/3600 * to_kelvin(n.bottom_temp)
                objective_node_energy = kWh_top + kWh_midlle + kWh_bottom
                nodes_with_same_top = [min(self.g.nodes[n.time_slice], key= lambda x: abs(x.energy-objective_node_energy))]
            knitted_node = nodes_with_same_top[0]
            
            self.feasible_branches[branch]['knitted_to'] = knitted_node
            self.feasible_branches[branch]['total_pathcost'] = knitted_node.pathcost + self.feasible_branches[branch]['hinge_cost']
